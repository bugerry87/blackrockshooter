...
{
    public class GB_AutoCam : GB_APivotBasedCameraRig
    {
		...
		[SerializeField] private bool m_FollowTurn = true;
        ...
        [SerializeField] private string m_DisableButton = "Fire2";
        [SerializeField] private GB_AUpdateMode postCorrection = null;

		public bool followVelocity { get { return m_FollowVelocity; } set { m_FollowVelocity = value; } }
		public bool followTilt { get { return m_FollowTilt; } set { m_FollowTilt = value; } }
		public bool followTurn { get { return m_FollowTurn; } set { m_FollowTurn = value; } }
        ...
        override protected void FollowTarget(float deltaTime)
        {
            ...
            var targetDir = m_Target.forward;
            var shortest = (transform.forward - m_Target.forward).sqrMagnitude;
            float next = 0;

            if(shortest > (next = (transform.forward - m_Target.right).sqrMagnitude) + 0.75f)
            {
                targetDir = m_Target.right;
                shortest = next;
            }
            ...
            if(shortest > (next = (transform.forward + m_Target.forward).sqrMagnitude) + 0.9f)
            {
                targetDir = -m_Target.forward;
                shortest = next;
            }
            ...
            if (m_FollowVelocity && Application.isPlaying)
            {
                ...
                if (targetRigidbody.velocity.magnitude > m_TargetVelocityLowerLimit)
                {
                    ...
                    targetDir = targetRigidbody.velocity.normalized;
                    targetUp = Vector3.up;
                }
                else
                {
            ...
            else if(m_FollowTurn && Application.isPlaying)
            {
                ...
                var currentFlatAngle = Mathf.Atan2(targetDir.x, targetDir.z) * Mathf.Rad2Deg;
            ...
            {
                targetDir.y = 0;
                if (targetDir.sqrMagnitude < float.Epsilon)
                {
                    targetDir = transform.forward;
                }
            }

            if(!m_FollowTurn || m_DisableButton.Length != 0 && (Input.GetButton(m_DisableButton) || Mathf.Abs(Input.GetAxis(m_DisableButton)) > 0))
            {
                //ignore
            }
            else
            {
                var rollRotation = Quaternion.LookRotation(targetDir, m_RollUp);
                // and aligning with the target object's up direction (i.e. its 'roll')
                m_RollUp = m_RollSpeed > 0 ? Vector3.Slerp(m_RollUp, targetUp, m_RollSpeed*deltaTime) : Vector3.up;
                transform.rotation = Quaternion.Lerp(transform.rotation, rollRotation, m_TurnSpeed*m_CurrentTurnAmount*deltaTime);
            }

            if(postCorrection != null && postCorrection != this)
            {
                postCorrection.ManualUpdate(updateType);
            }
        }
    }
}
